...
## 주요 기능
- **자동 인코딩 인식**: Shift-JIS (일본어), JIS (일본어), EUC-KR (한국어), CP949 (한국어), Johab (한글 조합형), GBK (간체 중국어), GB18030 (중국어 표준), Big5 (번체 중국어) 등을 자동으로 감지합니다.
...
- **파일 합치기 (merged.txt)**: "파일 합치기" 옵션을 체크하면 변환된 모든 파일이 하나로 합쳐진 `merged.txt` 파일이 생성됩니다.
- **이름순 정렬**: 파일 합치기 시 파일 이름순으로 정렬하여 합칩니다.
- **대상 확장자**: `.txt`, `.html`, `.htm` 파일을 지원합니다.

## 사용 방법
1. `EncodingConverter.exe`를 실행합니다.
2. (선택 사항) 파일을 하나로 합치고 싶다면 하단의 **"파일 합치기 (merged.txt)"** 체크박스를 활성화합니다.
3. 변환하고자 하는 파일이나 폴더를 프로그램 창 위로 드래그 앤 드롭합니다.
4. 변환이 완료되면 "변환 완료!" 또는 "성공" 메시지가 표시됩니다.
